...
const (
	_ int = iota // gives the following constants incrementing numbers as values - "_" takes the 0, the others 1-7
	LOWEST
	EQUALS      // =
	LESSGREATER // > or <
	SUM         // +
...
func (p *Parser) peekPrecedence() int {
	if p, ok := precedences[p.peekToken.Type]; ok { // if this (the next/peeked token) token exists in the precedences dict/map, return its precedence
		return p
	}

	return LOWEST // else return the lowest value in the precedences map
}

func (p *Parser) curPrecedence() int {
	if p, ok := precedences[p.curToken.Type]; ok { // does the same as above but for the current token
		return p
	}
...
func (p *Parser) parseReturnStatement() *ast.ReturnStatement {
	...
	// TODO: skipping expressions until encounter a semicolon
